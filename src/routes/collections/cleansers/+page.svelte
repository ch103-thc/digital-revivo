<script>
    ...
    // Extract unique brands from cleansers - now using the brand property
    const brands = [...new Set(cleanserCards.map((card) => card.brand))].sort();

    // Extract unique categories from cleansers
    const categories = [...new Set(cleanserCards.map((card) => card.category))];
    ...
    $: filteredProducts = cleanserCards
...
<section class="cleansers-page">
    ...
    <nav class="breadcrumb">
        ...
        <span class="separator">/</span>
        <span class="current">Face Cleansers</span>
    </nav>

    <!-- Page Title -->
    <div class="header-section">
        <h1>Face Cleansers</h1>
        <p class="product-count">
            {#if hasActiveFilters}
                {sortedProducts.length} of {cleanserCards.length} products
            {:else}
                {cleanserCards.length} products
            {/if}
        </p>
    </div>
    ...
    <div class="description">
        <p>
            Korean facial skin cleansing products are second to none. Among
            these high-quality products, you can definitely find something
            suitable for every skin type.
        </p>
        <p>
            The Korean skin care routine emphasizes double cleaning, when in the
            evening the face is first cleaned of make-up, dirt, sebum and
            sunscreen with an oil-based cleansing product or with micellar
            water. After this, the skin itself is carefully cleansed with a
            water-based cleansing product suitable for your skin. In the
            mornings, the face is washed either with just water or with a
            water-based cleanser (those with very dry skin can also try using a
            cleansing balm).
        </p>
    </div>
